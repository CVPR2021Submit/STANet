...
class att_Net(nn.Module):
    ...
    def forward(self, audio, video):
        ...
        layerAQuality = self.Afc(layerA_p)
        ...
        # 
        layerAV = F.relu((torch.softmax(layerAQuality, dim=1)[:, 1].unsqueeze(1).unsqueeze(2).unsqueeze(3) * (F.sigmoid(attention) * layerV_4)) + layerV_4)
